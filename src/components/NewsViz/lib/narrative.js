...
import { wrangle } from '../utils/utils.js';
import {
  SCENE_WIDTH,
  DEFAULT_HEIGHT,
  GROUP_MARGIN,
...
function Narrative(storyTimeline) {
  let narrative,
    scenes,
    characters,
    introductions,
  ...
  narrative.init = async function () {
    originalFile = storyTimeline;
    fileId = 'storyTimeline';
    ...
    wrangledNarrative.forEach(function (scene) {
      scene.characters.forEach(function (character) {
        try {
          ...
          character.width = textNode?.getComputedTextLength()
            ? textNode.getComputedTextLength() + 10
            : 100;
        } catch (e) {
          console.warn(`Failed to compute text width for "${character.name}"`, e);
          character.width = 100;
        }
