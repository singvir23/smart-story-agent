...
const ANTHROPIC_TIMEOUT_MS = 20000; // 20 seconds
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Wrap the main logic in a Promise.race
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            ...
                            if (response.status === 403) {
                                return NextResponse.json({ error: 'Failed to fetch article: Access denied (403). The site may block automated requests.' }, { status: 403 });
                            }
                            if (response.status === 404) {
                                return NextResponse.json({ error: 'Failed to fetch article: Not Found (404). Check the URL.' }, { status: 404 });
                            }
                            return NextResponse.json({ error: `Failed to fetch article: ${response.status} ${response.statusText}` }, { status: response.status });
                        }
                        ...
                        let errorMessage = 'An unknown fetch/parse error occurred.';
...
JSON Structure:
{
  ...
  "factSections": "(array of objects) <<< IMPORTANT: Generate this by segmenting the article. >>>
      ...
      4.  Each object MUST have:
          ...
          -   \\"content\\": \\"(string) The exact text of this 150-word segment from the article. DO NOT summarize or rephrase. This field must contain the original, verbatim text of the segment.
              All characters from the original segment, including newlines, tabs, quotes (single and double), backslashes, and other special characters, MUST be preserved.
              When you generate the JSON, ensure this text is correctly encoded as a JSON string. For example:
              *   A newline character in the original segment's text should be encoded as \\\\n in the JSON string.
              *   A tab character in the original segment's text should be encoded as \\\\t in the JSON string.
              *   A backslash character (\\\\) in the original segment's text should be encoded as \\\\\\\\ (two backslashes) in the JSON string.
              *   A double quote character (") in the original segment's text should be encoded as \\\\\\"(a backslash followed by a double quote) in the JSON string.
              *   Single quotes (') should be preserved as is (they do not require escaping in JSON strings when double quotes are used to delimit the string).
              The critical point is that the final JSON string for this field, when parsed by a standard JSON parser, must result in a string that is identical to the original article segment, including its internal newlines, tabs, and other special characters.\\"
      ...
      9.  CRITICAL: When handling the content field, ensure that:
          * All original text is preserved exactly as written
          * All original formatting is maintained (including whitespace like newlines and tabs)
          * All original quotes (both single and double) are preserved
          * The text is correctly JSON-escaped so it can be parsed back to its original form.",
...
6.  **BASE ON TEXT ONLY:** Do not add external information. Follow instructions for missing data. If SPICE scoring is not feasible, return null for 'spiceScore'.`;
                    ...
                    // --- Step 3: Call Claude API with timeout ---
                    try {
                        const claudeResponse = await Promise.race([
                            ...
                            new Promise((_, reject) => 
                                setTimeout(() => reject(new Error('Claude API timeout')), ANTHROPIC_TIMEOUT_MS)
                            )
                        ...
                        if (!claudeResponse.content || claudeResponse.content.length === 0 || claudeResponse.content[0].type !== 'text' || !claudeResponse.content[0].text) {
                            ...
                            return NextResponse.json({ error: 'Received an unexpected or empty response from the analysis service.' }, { status: 500 });
                        }
                        ...
                        return NextResponse.json({ 
                            error: 'The analysis service timed out or encountered an error. Please try again.' 
                        }, { status: 504 });
                    }
