...
interface StoryData {
        ...
        imageUrls?: string[];
        originalUrl: string;
        spiceScore?: SpiceScoreData | null; // Made optional for non-edit mode
        similarityScore?: number; // Made optional for non-edit mode
}
...
export async function POST(req: Request) {
        ...
        try {
                ...
                const { articleUrl, editMode } = body;
                ...
                // --- Step 2: Prepare Prompt for Claude (with conditional SPICE scoring) ---
                ...
                // Build the JSON structure based on editMode
                const spiceScoreField = editMode ? `"spiceScore": "(object or null) <<< NEW: Analyze the article text according to the SPICE rubric below and provide the scores. If the article is too short or lacks substance for a meaningful score, return null for this entire 'spiceScore' field. >>>
    {
      "s": (number) Scannability score (1-5),
      "p": (number) Personalization score (1-5),
      "i": (number) Interactivity score (1-5),
...
--- End SPICE Rubric ---` : '';
                ...
                const prompt = `Analyze the following article text and provide a structured summary${editMode ? ' AND a SPICE score' : ''} IN VALID JSON format ONLY.
...
JSON Structure:
{
  ...
  "factSections": "(array of objects) <<< IMPORTANT: Generate this by segmenting the article into EXACT 150-word chunks. >>>
      ...
      8.  IMPORTANT: Do not truncate or remove any content from the original article. Every word must be preserved in one of the sections."${editMode ? `,
  ${spiceScoreField}` : ''}
}
...
6.  **BASE ON TEXT ONLY:** Do not add external information. Follow instructions for missing data.${editMode ? ' If SPICE scoring is not feasible, return null for \'spiceScore\'.' : ''}`;
                // *** END OF MODIFIED PROMPT ***


                // --- Step 3: Call Claude API ---
                console.log(`Sending request to Claude API for ${articleUrl}${editMode ? ' (with SPICE scoring)' : ''}. Prompt length: ~${prompt.length} chars`);
                const claudeResponse = await anthropicClient.messages.create({
                        ...
                        max_tokens: 4000,
                        system: `You are an expert data extraction and analysis tool. Your sole purpose is to return valid, correctly formatted JSON based precisely on the user's instructions and the provided text. You output ONLY the JSON object requested, nothing else. Ensure all special characters within JSON string values are properly escaped according to JSON specification.${editMode ? ' Perform the SPICE analysis accurately based *only* on the provided text.' : ''} When creating 'factSections', adhere strictly to the 150-word segmentation rule and use the original text for content.`,
                ...
                try {
                        ...
                        // Basic validation for SPICE score structure if present and editMode is true
                        if (editMode && parsedData.spiceScore) {
                ...
                console.log(`DEBUG: Final storyData: Title='${storyData.title}', Author='${storyData.author || 'N/A'}', Date='${storyData.date || 'N/A'}', PrimaryImage='${storyData.imageUrl || 'N/A'}', AdditionalImages=${storyData.imageUrls?.length ?? 0}, Sections=${storyData.factSections.length}${editMode ? `, SPICE Score=${storyData.spiceScore?.total ?? 'N/A'}` : ''}`);
